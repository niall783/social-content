...
const fetcher = (url: string) => axios.get(url, {
  ...
  params: {
    timestamp: Date.now() // Add a timestamp to bypass cache
  }
}).then(res => res.data);
...
const FileList: React.FC = () => {
  ...
  const { data, error, mutate } = useSWR<{ filesFound: boolean; message: string; files: FileItem[] }>(
    '/api/aws/s3/list-s3-files',
    fetcher,
    {
  ...
  const handleDelete = async (): Promise<void> => {
    ...
    try {
      await axios.delete(`/api/aws/s3/delete-file?key=${encodeURIComponent(fileToDelete.key)}&timestamp=${Date.now()}`);
      refreshData(); // Trigger a re-fetch of the data
      closeDeleteModal();
    } catch (err) {
  ...
  const handleTranscribe = async (file: FileItem): Promise<void> => {
    ...
    try {
      const response = await axios.post('/api/transcribe', { fileKey: file.key, timestamp: Date.now() });
      alert(`Transcription completed. Transcription file: ${response.data.transcriptionFile}`);
      refreshData(); // Refresh the file list after transcription
    } catch (err) {
  ...
  if (error) {
    return <div className="text-center py-4 text-red-500">Failed to load files. Please try again later.</div>;
  }
...
// const FileList: React.FC = () => {
//   const { data, error, mutate } = useSWR<{ filesFound: boolean; message: string; files: FileItem[] }>('/api/aws/s3/list-s3-files', fetcher, {
//     refreshInterval: 5000 // Refresh every 5 seconds
//   });
//   const [isDeleteModalOpen, setIsDeleteModalOpen] = useState<boolean>(false);
//   const [fileToDelete, setFileToDelete] = useState<FileItem | null>(null);
...
//       alert(`Transcription completed. Transcription file: ${response.data.transcriptionFile}`);
//       mutate(); // Refresh the file list after transcription
//     } catch (err) {
//       console.error('Error transcribing file:', err);
//       // Handle error (e.g., show an error message to the user)
//     } finally {
